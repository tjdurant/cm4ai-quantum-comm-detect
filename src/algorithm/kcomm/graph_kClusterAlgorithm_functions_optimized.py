...
def runDwaveHybrid(Q, num_nodes, k, sub_qsize, run_label, run_profile, result):
  ...
  # Parse the string as a timedelta object
  time_obj = datetime.strptime(time_str, '%H:%M:%S.%f') - datetime.strptime('0:00:00.000000', '%H:%M:%S.%f')
  
  # Convert to total seconds as a float
  total_seconds = time_obj.total_seconds()

  result['wall_clock_time_seconds'] = total_seconds
